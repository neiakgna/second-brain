...
| **Store** | `store.py` | Persist chunks + vectors in SQLite via the `sqlite-vec` extension |
...
src/second_brain/
  ...
  embed.py      # text -> vector embedding
  store.py      # sqlite-vec persistence + vector search
  search.py     # query orchestration
tests/          # test suite — runs in under a second, no model download
...
- [ ] **v0.3** — Local LLM synthesis: answer questions, not just find chunks
- [ ] **v0.4** — Web UI
- [ ] **v0.5** — More sources: RSS feeds, browser bookmarks
